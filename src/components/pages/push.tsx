...
function Push() {
	...
	useEffect(() => {
		...
		if ("serviceWorker" in navigator && "PushManager" in window) {
			navigator.serviceWorker
				.register("/sw-push.js", { scope: "/push" })
	...
	const NotifyMeButtonHandler = async () => {
		...
		const subscription = await registration!.pushManager.getSubscription();
		fetch(
			"http://localhost:3002/push/send/5e430c04-3186-4560-bdb2-6ecf691047a3",
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	return (
		<div className="content">
			...
			<main className="slack-auth-main"></main>
			<Button
				child="subscribe"
			...
			<Button
				child="notify all"
				onClick={NotifyAllButtonHandler}
				disabled={notifyAllButton}
			/>
		</div>
	);
}
